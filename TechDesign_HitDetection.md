...
- **피격 시 아이템 드랍:** 보유 무기/버프를 바닥에 드랍 → `LootPod Interruption`과 연동 (→ `TechDesign_LootPod.md`)
...
HitDetection Processor가 판정을 내리기 전에 타겟의 `FParryStateFragment` 활성 여부를 먼저 확인. 패링이 활성화되어 있고 방향 조건을 만족하면 피격 처리 대신 패링 처리로 전환. (→ `TechDesign_ParrySystem.md`)
