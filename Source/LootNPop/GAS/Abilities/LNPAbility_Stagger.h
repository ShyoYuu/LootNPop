 ...
 *
 * 트리거는 경직도 임계 돌파 2종뿐이다(Stagger.Light/Heavy). 패링도 전용 이벤트가 아니라
 * 경직도(LNPPoise::ApplyParryBreak)를 거쳐 여기로 들어온다.
 *
 * 그로기(Light)는 **지속 시간이 없다** — 경직도가 T1 아래로 자연회복할 때까지 이어지는 상태이고,
 * 종료는 ULNPPoiseProcessor가 FLNPStaggerCommand로 취소해 준다. 다운(Heavy)만 고정 시간이다.
 *
 ...
 *
 * ⚠️ 따라서 경직 길이와 몽타주 길이는 데이터상 분리돼 있다. 다운 길이는 ULNPSettings의
 *    PoiseDownLockSeconds가, 몽타주는 Chooser(LNP.Montage.Situation.Stagger)가 각각 정한다.
 */
UCLASS()
