...
public:
	 ...
	 *
	 * **원칙: 프롬프트 수용 범위가 넓을수록 우선순위가 낮다.** 범위가 넓은 대상이 이기면 그 넓은 영역
	 * 안의 좁은 대상들이 **전부 무력화되지만**, 좁은 대상이 이기면 몇 걸음만 물러나도 넓은 쪽을 쓸 수 있다.
	 * 손해가 비대칭이므로 좁은 쪽에 우선권을 준다.
	 *
	 * | 대상 | 값 | 수용 범위 |
	 * |:---|---:|:---|
	 * | LootPod       |  30 | 250cm + Pod 전방 60° (가장 좁다) |
	 * | LootDice      |  20 | 250cm + 캐릭터 전방 140° |
	 * | SpringLauncher|  10 | 250cm, 각도 제한 없음 |
	 * | GrappleAnchor | -10 | 수십 m + 화면 중앙 (가장 넓다) |
	 *
	 * Pod·Dice가 위인 것은 범위 외에 기획 의도도 겹친다 — Pod은 게임의 핵심 목표, Dice는 보상이다.
	 * Pod이 Dice를 가리는 구간은 좁다: Pod은 Idle일 때만 프롬프트를 띄우므로(WantsInteractionPrompt)
	 * 루팅 중이거나 터진 Pod은 자기가 뱉은 보상 주사위와 경쟁하지 않는다.
	 *
	 * 기본값 0은 "근접 장치인데 서열을 아직 안 정함" 자리다 — 앵커보다는 위라 새 장치가 조용히 묻히지 않는다.
	 */
