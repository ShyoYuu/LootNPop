...
void ULNPWeaponTraceHitDetectionProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	AttackerQuery.ForEachEntityChunk(Context, [&](FMassExecutionContext& Ctx)
	{
		...
		for (int32 i = 0; i < Ctx.GetNumEntities(); ++i)
		{
			 ...
			 * **각 머신에서 로컬로 탐색**했기 때문이다. 게스트는 보간된(과거) 적 위치를, 서버는 권위 현재
			 * 위치를 읽어 서로 다른 지점으로 워프하고, 그 상태에서 판정이 적을 되감으면 워프가 끌어다 놓은
			 * 자리에서 그만큼 더 멀어진다. 탐색 결과를 소유 클라이언트가 발동 RPC로 보내게 해(`FLNPMeleeAssistTargetData`)
			 * 그 원인은 제거했으나, 재측정에서도 순손실이 남았다 — 플레이어의 리드 조준(보간 지연을 스스로 갚는 행위)과
			 * 이중 보상이 되는 것으로 추정한다(미검증).
