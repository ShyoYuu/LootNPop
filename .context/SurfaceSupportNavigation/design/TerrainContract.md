...
### 섬과 지각을 잇는 경사로 제작 방법(D-051)

두 방법 모두 허용한다. 옥탄트마다 의도에 맞게 고른다. `Meadow_00` 섬 A에 둘 다 있다(극 쪽 블렌더, 적도 쪽 지각 일체형).

| | 별도 메시(블렌더) | 지각 일체형(`LNP.MeshTerrain.RadialRamp`) |
|:---|:---|:---|
| 제작 | `Scripts/Blender/GenerateFloatingIslands.py` 파라미터 → FBX 임포트 | 콘솔 명령이 지각 정점을 목표 반지름으로 올린다. 지면을 올리기만 한다 |
| 모양 | 반듯한 인공 구조물. 몸체를 Boolean으로 파내 터널·방을 만들 수 있다 | 지형이 솟은 형태, 옆면 경사로 지형에 녹는다 |
| 컴포넌트 | 윗면(`Support+Blocker+Static`)과 몸체(`Blocker+Static`) 두 개. 발끝·섬 쪽 이음매가 컴포넌트 경계다 | 지각 컴포넌트 하나. 발 쪽 이음매가 없다 |
| PCG 프랍 | 경사로 위에 생기지 않는다 | 지각이라 언덕 위에도 뿌려진다. 통행을 막으면 PCG 제외 구역이 필요하다 |
| 수정 | 파라미터 수정 후 재임포트(같은 이름 덮어쓰기 불가, 새 이름으로 교체) | 누적 적용이므로 원본 지각에서 다시 만든다 |

- 섬 측의 접합은 경사로 윗면을 섬 윗면보다 1cm 낮게 두어 z-fighting 없이 1cm 단차로 잇는다.
- 경사로에 판 터널·방은 시험용 간이 동굴이다. 정식 지하 공간은 §6 동굴 키트를 따른다.
- 섬·국소 지형을 Mesh Terrain Sculpt로 다듬을 때는 Plane 기준면을 쓴다. Sphere 기준면은 섬 메시를 편집하지 못하고 지각도 반지름 30,000cm 안쪽만 편집한다(`../research/MeshTerrain.md` 마지막 절).
...
- 부유섬은 옥탄트 LVI 안에 직접 배치하는 옥탄트의 일부이며, 옥탄트와 별도로 조합하지 않는다(D-048). 기본 보행면은 월드 구와 중심을 공유하는 구면 곡면이다.
