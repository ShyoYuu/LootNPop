...
현재 배치는 Phase 3 exact query 검증에는 문제가 없지만 옥탄트별 베이크 검증에는 맞지 않는다.

- 모든 fixture가 적도 `Z=0`에 있고 각 패치가 Z ±750cm로 그 평면을 걸친다. 적도는 옥탄트 이음매 평면이므로 Phase 4 이후에는 모든 사례가 이음매 사례가 된다.
- "옥탄트 seam" 사례의 `(0, 1, 0)`은 이음매 한 줄이 아니라 4개 옥탄트가 만나는 꼭짓점이다.
- 맵이 일반 레벨이라 LVI·slot 회전 경로를 타지 않는다.

변경 계획:

| 시점 | 변경 |
|:---|:---|
| Phase 3 | 상태형 기둥·움직이는 패널·파괴 바닥 fixture를 Placement Marker와 서버 스폰 요소로 전환 |
