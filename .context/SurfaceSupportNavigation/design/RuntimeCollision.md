...
- 부하 기준선의 프레임 판정은 패키지 호스트 `-nullrhi`의 서버 CPU 프레임으로 한다. 렌더링 호스트 프레임은 측정 머신 GPU에 좌우되므로 기준선으로만 기록한다. 에디터 바이너리 `-game`은 에디터 전용 디버그 드로우가 켜져 있고, `-nullrhi`에서는 수명 0 디버그 라인이 비워지지 않아 쌓이므로 프레임 판정에 쓰지 않는다.
