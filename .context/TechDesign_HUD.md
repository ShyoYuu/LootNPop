...
### 11.7 첫 소비자는 HP 바가 아니라 **락온 마커**다 (2026-09-06)

락온 마커도 지금 `UWidgetComponent`다(§11.2의 "적 1마리당 2개" 중 하나). 그래서 순수 엔티티에
락온을 걸면 **표시할 방법이 없다는 같은 벽**에 부딪힌다 —
→ [TechDesign_TargetQuery.md](TechDesign_TargetQuery.md) §7의 3단계.

**ISM 인스턴스·Niagara로 엔티티용 마커만 따로 만드는 안은 기각했다.** 지금 마커가 이미 UMG라
그렇게 하면 *Actor용 위젯 마커*와 *엔티티용 월드 마커* 두 갈래를 영구히 유지하게 된다 —
이 프로젝트가 순수 엔티티 도입 이후 반복해 밟은 바로 그 함정(Actor 경유 경로의 갈라짐)이다.
락온 마커는 성격상 UI라(거리와 무관하게 또렷하고 일정한 크기) 스크린 스페이스가 자연스럽기도 하다.

⚠️ **락온 마커는 §11의 비싼 부분을 하나도 쓰지 않는다.** 그래서 HP 바를 기다릴 이유가 없고,
오히려 **형틀(§11.6)의 첫 소비자로 먼저 태우는 편**이 낫다:

| 항목 | HP 바 | 락온 마커 |
|:---|:---|:---|
| §11.3 Health 복제 | **필수 선결** | 불필요 — 체력을 표시하지 않는다 |
| §11.4 개수 제한·히스테리시스 | 필수 | 불필요 — 항상 정확히 1개 |
| §11.5 가림 트레이스 | 진짜 작업 | 불필요 (아래) |
| §11.6 위젯 형틀 + 스크린 투영 | 필요 | **필요 — 이것만 쓴다** |

- 락온 마커는 **가려지지 않는 편이 옳다.** 내가 지목한 대상이 엄폐물 뒤로 들어갔다고 표식이
  사라지면 곤란하다. §11.2가 "잃게 된다"고 적은 뎁스 가림은 HP 바에만 해당한다.
- 옮기면 매 프레임 카메라 빌보드 회전(`LNPEnemyCharacter.cpp:73-87`)도 함께 없어진다.

⚠️ **원소가 1개라고 위젯 API를 스칼라로 좁히지 말 것.** §11.6이 정한 대로 **배열**
(스크린 좌표·비율·알파·스케일)로 두어야 HP 바가 나중에 그대로 올라탄다.

⚠️ **`ALNPEnemyCharacter::LockOnMarkerComponent`는 이미 제거됐다 (2026-09-06).**
...
가드를 씌우는 대신 **경로를 지웠다.** 어차피 이 작업에서 삭제 예정이었고, 남겨 두면 두 종류의 적이
서로 다른 피드백을 받는 갈라짐이 그대로 유지된다. 그동안의 확인 수단은 `LNP.LockOn.Debug` 디버그 드로우다.
...
⚠️ **락온 마커(§11.7)는 이 조건에 매이지 않는다** — §11.3~11.5를 쓰지 않으므로 독립적으로 착수할 수 있고,
형틀을 먼저 실전 검증해 준다.
---
