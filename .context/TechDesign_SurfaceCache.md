...
- ⚠️ **수직 단차는 표현할 수 없다 — 이 캐시의 구조적 한계이자, 지형 생성이 지켜야 할 규약이다.**
  `CellSpacing`(현재 100cm) 균일 격자 + 바이리니어는 **한 셀 안에서 완결되는 1m 낙차를 표현하지 못한다.**
  표현하려 하면 절벽 위 셀(반지름 작음)과 아래 셀(반지름 큼)을 섞어 **실제 지면이 없는 중간 높이**를
  만들고, 소비처가 그 값으로 강제 스냅하므로 그 자리에 있는 것이 벽면 안에 파묻힌다.
  실측(2026-09-10): 이음매에 폭 2m·깊이 1m 도랑이 있었을 때 최대 **±140cm** 오차가 났고,
  도랑을 없앤 뒤 구면 전체 평균 오차는 **2.2cm**로 떨어졌다.
  **해상도로 풀리지 않는다** — `CellSpacing`을 절반으로 줄이면 오차도 절반일 뿐이고, 25cm까지 낮추면
  샘플이 19.7M(약 630MB)이 된다.
  ⚠️ **임계값으로 "불연속이면 최소 반지름 사용" 같은 분기를 넣는 것도 답이 아니다.** 100cm 격자에서는
  걸어 다닐 수 있는 45° 경사도 인접 셀 간 반지름 차이가 이미 ~100cm라, **1m 절벽과 45° 경사가
  격자 입장에서 같은 신호**다. 임계값을 낮추면 정상 경사가 계단이 되고 높이면 절벽을 못 잡는다.
  → 따라서 **절벽 지형을 도입하려면 캐시 표현 방식 자체를 다시 설계**하거나, 지형 생성이 보장할
  **최대 경사를 규약으로 못박아야** 한다. 실패 방향을 매몰이 아니라 부양으로 돌리는 안전장치
  (접지 스냅에 스텝 제한을 두고 초과 시 기존 공중 물리 경로로 넘김)는 그와 별개로 가치가 있다.
