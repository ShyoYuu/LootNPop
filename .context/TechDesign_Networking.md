...
그래서 되감기에 **대상의 복제 LOD 갱신 주기**(High 0.1 / Medium 0.2 / Low 0.3초)를 더한다. 거리 대역은
뷰어(= 공격자) 기준으로 판별한다(`LNP::Replication::GetReplicationLODByDistanceSq`).
A/B용 CVar은 `LNP.HitDetection.CompensateInterpolationLag`.

```
되감기 = clamp(RTT/2, 0, MaxPingRewindSeconds 0.2) + 대역 갱신 주기     (최대 0.5초)
```

⚠️ **상한은 핑 항에만 건다. 두 항을 한 상한으로 묶으면 안 된다.** 처음엔 기존 200ms 총량 상한을 그대로
뒀는데, 그러면 **Medium 이상에서 대역항(0.2초)만으로 포화해 핑 보정이 통째로 잘려나간다** — 상한의 원래
목적과 정반대다. 실측에서 Medium 표본의 되감기가 전부 0.200 고정으로 나와 드러났다(로컬 핑에서조차 포화).
두 항은 성격이 다르다:
...
합의 최댓값 0.5초가 곧 **히스토리 버퍼 예산**이다 — `MaxSamples`를 5→11로 키워 `(11-1)×50ms = 500ms`를
덮는다(`LNPPositionHistoryFragment.cpp`의 static_assert가 이 관계를 못 박는다). 모자라면
`GetInterpolatedLocation`이 **조용히** 최고령 샘플로 클램프해 되감기가 소리 없이 짧아진다.
대가는 대역폭이 아니라 **청크 밀도**다: 프래그먼트가 176B→368B로 커지고, 이 프래그먼트는 적 아키타입에서
이미 큰 축이며 그 아키타입을 14개 프로세서가 매 프레임 순회한다.

⚠️ **되감기는 대역폭을 쓰지 않는다.** 히스토리는 서버 로컬 링버퍼이고 클라에서는 기록조차 하지 않는다
(`LNPPositionHistoryProcessors.cpp`의 넷 모드 가드). 같은 오차를 *갱신 주기를 줄여서* 고치는 길도 있지만
그쪽은 버블 송신량이 직격이다 — 되감기 쪽이 압도적으로 싸다.

⚠️ **가산항이 주항이다.** 실측 교전 거리 24~47m는 복제 LOD Medium 대역이라 가산항이 0.2초인데,
정상 회선(RTT 40ms)의 `RTT/2`는 0.03초다. 적 이동 속도 600cm/s 기준 **120cm — 캡슐 지름(70cm)의 1.7배**이고,
**핑이 0이어도 그대로 남는다.** 도입 전 되감기는 완벽한 회선의 게스트에게조차 전체 오차의 9%만 갚고 있었다.
...
플레이어·LootPod은 애초에 Mass 버블로 위치가 오지 않고(`PushSmoothingTarget`의 폴백), 승격 Actor는
스무딩에서 제외된 뒤(`CurrentRepresentation` 분기) 전방 예측으로 현재까지 온다. 그래서 **PvP 되감기는
바뀌지 않았고**, 늘어난 되감기로 "엄폐물 뒤에서 맞았다"를 겪는 쪽은 NPC뿐이다 — 이 비대칭이 이 보정을
싸게 만든다.

⚠️ **게이트는 "복제를 거치는가"이지 "지연이 큰가"가 아니다.** 리슨 호스트는 Ping이 0이라 되감기도 0이지만
화면 자체가 복제를 거치지 않고, 반대로 LAN 게스트는 Ping이 0에 가까워도 보간 지연은 그대로 있다.
그래서 `CachedRewindSeconds > 0`으로 대신할 수 없고, 서버의 `!IsLocallyControlled()`로 가른다
(원거리는 발사 시점에 `bInstigatorIsRemoteClient`로 캐싱, 근접은 판정 프레임마다 직접 조회).

⚠️ 남은 한계 둘: `GetPingInMilliseconds()`는 순간값이 아니라 엔진이 창 단위로 평균낸 값이라 급격한 지연
변동을 따라가지 못한다. LOD 대역 판별도 **명목** 주기라, 일회성 전이(`IsOneShot`)가 게이트를 우회해 실제
수신 간격이 짧아진 구간에서는 살짝 과대 추정한다.
