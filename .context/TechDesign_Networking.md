...
### 4.9 LocalPredicted 어빌리티의 "지연 후 확정"은 타이머 콜백이 아니라 EndAbility에 둔다 (2026-09-19)

**두 번 밟았다.** 재장전(탄이 안 채워짐)과 스프링 런처(사출이 안 됨)가 같은 버그다.

**증상.** 리슨 서버 호스트는 멀쩡한데 **게스트만** 동작하지 않는다. 게스트 화면에서는 "한순간 날아갈 것처럼
깜박이더니 제자리"다. 게스트 로그에는 확정 로그가 `authority=0`으로 찍히는데 **서버 로그에는 그 폰에 대한
`authority=1` 줄이 없다.** 발동 자체는 성공했고 기각 로그도 없다.

**원인.** `LocalPredicted`는 소유 클라와 서버가 **각자 자기 시계로** 지연 타이머를 돌린다. 그런데 서버는
자기 타이머와 **소유 클라의 종료 통지 중 먼저 온 쪽으로 끝나고, 클라 통지가 먼저 오면 서버 타이머 콜백은
영영 불리지 않는다.** 확정 행동을 타이머 콜백에 두면 그 경로에서 통째로 증발한다. 소유 클라는 예측으로
실행했으므로 다음 권위 상태가 그것을 롤백한다 — 그래서 "깜박이고 제자리"로 보인다.
호스트가 멀쩡한 이유는 자기가 서버라 이 경쟁 자체가 없기 때문이다. **증상이 게스트 전용이면 이걸 먼저 의심하라.**

**해결.** 확정 행동을 `EndAbility`로 옮긴다 — 어느 쪽 타이머가 이기든 양쪽에서 반드시 불린다. 가드 두 개가 짝이다:
`bWasCancelled`면 실행하지 않고(경직 등으로 끊긴 경우), **경과 시간 하한**(`MinCompletionRatio`, 기본 0.8)으로
조기 종료 통지가 즉시 확정으로 둔갑하는 것을 막는다.
선례: `ULNPAbility_Reload::EndAbility`, `ULNPAbility_SpringLaunch::EndAbility`.

### 4.10 레이어드 무브의 잔여 속도는 **큐잉된 새 무브보다 먼저** 적용된다 (2026-09-19)

네트워크 이슈는 아니지만 위와 증상이 겹쳐 진단을 흐린다 — 함께 적어 둔다.

런처의 정렬 이동(`FinishVelocityMode = SetVelocity`, 값 0)과 사출을 **같은 시점**에 예약했더니 사출이
전혀 먹지 않았다. `FLayeredMoveGroup`은 끝난 무브의 잔여 속도를 캐릭터에 적용한 **뒤에** 큐잉된 무브를
활성화한다(`LayeredMove.cpp::GatherResidualVelocitySettings` → 큐 활성화 순서). 그래서 사출이 실린 틱이
정렬의 잔여 0으로 덮였다. **지연 후 임펄스를 주는 구조에서는 선행 무브를 반드시 먼저 끝내고 간격을 둔다.**
(`FLayeredMove_Launch`의 `DurationMs = 0`은 엔진 기본값이고 한 틱 적용이 정상이다 — 여기를 의심하면 헛짚는다.)
