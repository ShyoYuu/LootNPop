...
- 위젯 계층은 `UCommonActivatableWidgetStack`(메뉴 열기/닫기) + `UCommonActivatableWidgetSwitcher`(탭 전환) 조합. `UCommonTabListWidgetBase`가 L1/R1 탭 이동을, `UCommonBoundActionBar`가 입력 방식(키보드↔게임패드)에 따른 아이콘 자동 전환을 담당.
...
| [CharacterMovement 기술 설계](TechDesign_CharacterMovement.md) | 구형 중력 3모드, 곡률 보정, 컨트롤 회전 파이프라인, 카메라 리그 노드 순서 제약, 질주·가드·대시 시스템 |
| [전투 Animation 기술 설계](TechDesign_CombatAnimation.md) | Motion Matching 로코모션, 무기별 Linked Anim Layer 교체, Aim Offset·왼손 Two Bone IK·Guard 자세 분기 구현 완료 |
...
| [인게임 메뉴 기술 설계](TechDesign_InGameMenu.md) | CommonUI 위젯 계층, Back 전파 규칙, 스탯 합/곱 분해, 입력 배선과 필수 설정 |
