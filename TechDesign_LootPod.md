...
**구현 방향:** HitDetection 시스템(→ `TechDesign_HitDetection.md`)에서 피격 판정 시 해당 플레이어 엔티티의 `FLNPPlayerLootingTag`를 제거. 태그 제거가 트리거가 되어 `ULNPLootingProcessor`가 다음 프레임에 Idle로 복귀.
