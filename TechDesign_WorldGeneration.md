...
> 단일 PCG 볼륨 기반 전역 생성 방식은 스케일링 한계로 기각. 상세 이력은 `DiscardedApproaches.md` 참조.
...
**결정론적 보장:** 서버 GameMode가 `OctantGenSeed`를 GameState에 쓰고 복제 → 클라이언트도 동일한 seed로 `StartWorldGeneration()` 호출 → 동일한 8개 배치. (상세: `TechDesign_InitSequence.md`)
