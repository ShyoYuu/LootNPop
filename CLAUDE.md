...
| .context/TechDesign_WorldDevice.md | 월드 장치(그래플 앵커·스프링 런처) — 예측 경로 vs 발동 요청 경로, 판정 3분할(권한/조준/시뮬 권위), 앵커 ID를 싣는 이유, 레이어드 무브 2개 구성, 시드 결정론 배치와 절벽·단차 기각 |
...
| .context/TechDesign_HitDetection.md | 근접 Swept Volume·원거리 Line Segment 판정, 판정 캡슐 중심 규약 단일 헬퍼, 조준 원본 단일화와 수직 이탈 게이트, 조준점 깊이(적 엔티티 질의), 연출과 데미지의 분리·ASC는 전송 수단, 스플래시 감쇠, 지면 착탄·수명 만료 폭발(판정 소유권 단일화), 트레일 해제 소유권(파괴 시점 단일화) |
